...
class GameConfig:
    ...
    """
    def __init__(self, width=8, height=8, num_mines=10, auto_expand_clear_areas=False):
        self.width = width
        self.height = height
        self.num_mines = num_mines
        self.auto_expand_clear_areas = auto_expand_clear_areas
...
class Game:
    ...
    def __init__(self, config, mines=None):
        ...
        self.auto_expand_clear_areas = config.auto_expand_clear_areas
        self.num_moves = 0
    ...
    def _place_mines(self):
        ...
        while len(locations) < self.num_mines:
            ...
            # Or comment the two lines above and place mines as you prefer!
            # x = self.width - 1
            # y = self.height - 1
    ...
    def _update(self, x, y):
        ...
        if self.auto_expand_clear_areas:
            stack = [(x, y)]
            while len(stack) > 0:
                (x, y) = stack.pop()
                for dx, dy in itertools.product([-1, 0, 1], repeat=2):
    ...
    def __next__(self):
        ...
        if not self.game.game_over:
            ...
            if result.status == GameStatus.PLAYING:
                ...
                print("Final flags -> ", self.ai.flags)
